...
if __name__ == '__main__':
    ...
    number_try=0
    folder_path = './data_stolen/At The Beginning Of The Novel, The Heroine Begged The Retired Villain – Wonder Novels'  # Thay đổi đường dẫn đến thư mục của bạn tại đây
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.endswith('.txt'):  # Chỉ xử lý các tệp tin có phần mở rộng .txt
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                print('ĐƯỢC CHIA RA LÀM',num_of_part)
                ...
                # for i in range(1):
                    ...
                    try:
                        ...
                        for data in chatbot.continue_write(conversation_id = conversation_id, auto_continue  = True):
                            ...
                            print(message, end="", flush=True)
                            prev_text = data["message"]
                        with open(os.path.join('GPTcommunication','text'+str(index)+'.txt'), "w", encoding='utf-8') as file:
                            file.write(response_bk+response)
                        file_list.append(os.path.join('GPTcommunication','text'+str(index)+'.txt'))
                        #when everything done then
                ...
                concatenate_files(file_list,os.path.join('tonghop',filename))
