...
function get_namechapters(inputString) {
	...
	const cleanedMatches = matches.map((match) => match.replace(/,\s*$/, "").trim());
...
async function editNovel() {
	...
	const postData = JSON.stringify({
		id: extractedID,
		name_chaps: name_chaps ? get_namechapters(sessionStorage.getItem("name_chapters")) : [],
...
$(document).ready(function () {
	...
	$(document).on("change", ".page5_chap .file-input", function () {
		const file = $(this)[0].files[0];
		if (file) {
			if (validateFile(file, true)) $(this).parent().find(".file-content").text(file.name);
		} else $(this).parent().find(".file-content").text("");
	});
