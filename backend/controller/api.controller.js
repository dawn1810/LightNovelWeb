...
const api_search_more = async (req, res) => {
	try {
	  ...
	  ten_truyen AS name,
	   ...
	   truyen.id_tac_gia = tacgia.id AND ten_truyen REGEXP '${search}' LIMIT 31 OFFSET ${30 * num_click}
	  ...
	  `);
        ...
        AND the_loai.ten_the_loai REGEXP '${search}' LIMIT 31 OFFSET ${30 * num_click}
        `);
				if (genres.length == 31) {
					genres_more = true;
					genres.pop();
	  ...
	  ten_truyen AS name,
	   ...
	   trang_thai AS status, so_luong_chuong AS no_chapters 
	   FROM truyen, tacgia WHERE 
	   truyen.id_tac_gia = tacgia.id AND ten_tac_gia REGEXP '${search}' LIMIT 31 OFFSET ${
					30 * num_click
				}
	  `);
	...
	try {
		...
		switch (update_day) {
			...
			case "3":
				query.update_date = `truyen.ngay_cap_nhat BETWEEN ${
					func_controller.getFirstAndLastDayOfWeek().firstDay
				} AND ${func_controller.getFirstAndLastDayOfWeek().lastDay}`;
				break;
			case "4":
				query.update_date = `truyen.ngay_cap_nhat BETWEEN ${
					func_controller.getFirstAndLastDayOfMonth().firstDay
				} AND ${func_controller.getFirstAndLastDayOfMonth().lastDay}`;
				break;
			case "5":
				query.update_date = `truyen.ngay_cap_nhat BETWEEN ${
					func_controller.getFirstAndLastDayOfYear().firstDay
				} AND ${func_controller.getFirstAndLastDayOfYear().lastDay}`;
				break;
		}
		...
		if (validConditions.length > 0) {
			...
			result = await queryAsync(
				`SELECT truyen.id AS _id, truyen.ten_truyen AS name, tacgia.ten_tac_gia AS author, truyen.anh_dai_dien AS image, truyen.trang_thai AS status, truyen.so_luong_chuong AS no_chapters FROM tacgia, truyen ${
					query.genres ? ", the_loai, the_loai_truyen" : ""
				} WHERE truyen.id_tac_gia = tacgia.id AND ` +
					sqlQuery +
					` ORDER BY ` +
					sort +
					` LIMIT 31 OFFSET ${30 * parseInt(skip)}`
			);
			// console.log(
			// 	`SELECT truyen.id AS _id, truyen.ten_truyen AS name, tacgia.ten_tac_gia AS author, truyen.anh_dai_dien AS image, truyen.trang_thai AS status, truyen.so_luong_chuong AS no_chapters FROM tacgia, truyen ${
			// 		query.genres ? ", the_loai, the_loai_truyen" : ""
			...
			result = await queryAsync(
				`SELECT truyen.id AS _id, truyen.ten_truyen AS name, tacgia.ten_tac_gia AS author, truyen.anh_dai_dien AS image, truyen.trang_thai AS status, truyen.so_luong_chuong AS no_chapters FROM tacgia, truyen ${
					query.genres ? ", the_loai, the_loai_truyen" : ""
				} WHERE truyen.id_tac_gia = tacgia.id ` +
					" ORDER BY " +
					sort +
					` LIMIT 31 OFFSET ${30 * parseInt(skip)}`
			);
		...
		res.writeHead(200, { "Content-Type": "applicaiton/json" });
		res.end(JSON.stringify({ novel: result, more_novel: more_novel }));
		// console.log(query);
