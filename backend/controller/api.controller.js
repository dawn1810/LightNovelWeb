			...
			for (let i = 0; i < f_result.length; i++) {
				if (func_controller.comparePassword(data.pass, f_result[i].mat_khau)) {
					...
					await queryAsync(
						...
						[id_user, data.usr, f_result[i].email, func_controller.hashPassword(data.pass)]
					);
...
const api_uploadNovel = async (req, res) => {
	...
	try {
		...
		if (account) {
			...
			VALUES (?,?,?,?,?,?,?,?)`,
			[novel_id, account.id, data.name, data.name_chaps.length, data.summary, avt_var, data.status, formattedDate]
			);
...
const api_editInfoNovel = async (req, res) => {
	...
	try {
		...
		SET 
			...
			anh_dai_dien = ?
		WHERE id = ?;`,
		[data.novel_name, data.novel_status, data.novel_descript, formattedDate, avt_var, data.id]);	
			...
			if (allowedMimeTypes.indexOf(req.files[0].mimetype) == 1) {
				...
				if (result) {
					...
					for (let i = 0; i < result.name_chapters.length; i++) {
						...
						VALUES (?,?,?,?,?)`, 
						[uuidv4(), novel_id, result.name_chapters[i], result.content_chapter[i], i+1]);
					}
