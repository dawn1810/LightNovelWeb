...
const api_uploadNovel = async (req, res) => {
  try {
    ...
    if (account) {
      ...
      const data_genre_id = await queryAsync("SELECT * FROM the_loai WHERE ten_the_loai IN (?)",[data.genres])
...
const api_editInfoNovel = async (req, res) => {
  try {
    ...
    if (account) {
      ...
      const data_genre_id = await queryAsync("SELECT * FROM the_loai WHERE ten_the_loai IN (?)",[data.novel_types])
      const ds_genre_id = data_genre_id.map((row) => row.id);
  ...
  try {
    ...
    if (allowedMimeTypes.indexOf(req.files[0].mimetype) == 1) {
      ...
      if (result) {
        ...
        const data_genre_id = await queryAsync("SELECT * FROM the_loai WHERE ten_the_loai IN (?)",[result.genre])
        const ds_genre_id = data_genre_id.map((row) => row.id);
...
const add_new_category = async (req, res) => {
  ...
  const id =  uuidv4();;
  try {
    const category_id = await queryAsync(`SELECT id FROM the_loai WHERE id=?`, [
      id,
    ]);

    if (category_id.length == 0) {
      await queryAsync(`INSERT INTO the_loai(id,ten_the_loai)VALUES(?,?);`, [
        id,
        category,
      ]);
      const newData = await queryAsync(`SELECT * FROM the_loai`);
